...
export default function SidebarSponsorAILN({
 ...
 return (
 <div
 className={`hidden fixed w-full h-full left-0 z-50 lg:flex lg:flex-col bg-white dark:bg-black dark:border-r dark:border-sidebar-border ${
 ...
 className="absolute -right-4 top-6 z-10 flex h-8 w-8 items-center justify-center rounded-full border bg-white shadow-sm dark:border-sidebar-border dark:bg-black "
 >
 <ChevronLeft
 className={`h-3 w-3 text-muted-foreground transition-transform ${isCollapsed ? "rotate-180" : ""}`}
 />
 </button>
 ...
 />
 {!isCollapsed && (
 <span className="text-xs font-semibold text-muted-foreground dark:text-sidebar-foreground">
 Dashboard Sponsor
 </span>
 )}
 </div>
 ...
 : "text-muted-foreground hover:bg-muted hover:text-black dark:hover:bg-white/5 dark:hover:text-white"
