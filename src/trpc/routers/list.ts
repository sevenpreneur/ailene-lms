...
export const listRouter = createTRPCRouter({
      ...
      const list = members
        .map((m) => {
          ...
          if (!last_active_at) {
            ...
            const daysAgo = now.diff(dayjs(last_active_at), "day", true);
        ...
        .sort((a, b) => {
          if (b.current_level.level_number !== a.current_level.level_number) {
            return b.current_level.level_number - a.current_level.level_number;
          }
  ...
  memberUseCaseLibrary: ailMemberProcedure.query(async (opts) => {
    ...
    const list = useCases.map((u) => {
      const submission = u.submissions[0] ?? null;
      return {
        id: u.id,
        name: u.name,
        description: u.description,
  ...
  practiceSubmissions: ailMemberProcedure.query(async (opts) => {
    ...
    const list = [
      ...promptRows.map((r) => ({
        ...
        reviewed_by: r.reviewed_by
          ? {
              id: r.reviewed_by.id,
              full_name: r.reviewed_by.user.full_name,
              avatar: r.reviewed_by.user.avatar,
            }
          : null,
        deadline: r.deadline,
        message: r.message,
    ...
    ].sort((a, b) => dayjs(b.submitted_at).valueOf() - dayjs(a.submitted_at).valueOf());
